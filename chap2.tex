...
If population activity in the brain represents possible hypotheses about the world, then we want to model it in a probabilistic, Bayesian framework.
In this section, we derive such a framework and shows that sparse coding approximately solves it.
...
% TODO: This sigma corresponds to the scale of the coefficients, not the same sigma as in the data. Just use lambda, say it is proportional to the variance. It should multiply the activation
\begin{equation}\label{eq:ch2_coefficient_prior}
    p(a) \propto \prod_{i}\frac{1}{\lambda} e^{\frac{-|a_{i}|}{\lambda}},
\end{equation}

%TODO:
%we're also assuming a Gaussian prior on the basis functions
% it's not from noise assumption
% move this down to where we derive the weight update rule
% in practice we approximate this prior by forcing the basis functions to be unit length after each update step, which is akin to projected gradient descent
...
% p(phi|s) is posterior, which is what we really want to find. add weight decay term
% Expectation exactly comes from integral
% usually people will do this with a monte-carlo type process
% instead of sampling from the coefficients a to integrate over the entire state space (or close to), we will take a single sample in the form of the MAP estimate.
...
\begin{figure}[h]
    ...
    \caption{\textbf{LCA architecture.} The LCA network includes feedforward driving connections, $\Phi$ as well as a lateral connectivity matrix, $G$. The dark arrows are connections for neuron $k$. All other connections are denoted by dotted lines.}
...
These are well matched to fits of the linear approximations of receptive fields of mammalian V1 simple cells obtained via spike-triggered averaging \parencite{vanHateren1998independent}.
...
These sub-threshold dynamics can result in a better solution than the standard ISTA framework, which turns on all units with a non-zero inner product with the input at the first inference step \parencite{rozell2008sparse}.
