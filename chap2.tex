...
As a followup to this study, \parencite{zhu2013visual} show that the LCA network also exhibits a variety of non-linear receptive field effects, including end-stopping, cross-orientation inhibition, and surround suppression.
...
In \parencite{vinje2000sparse}, the authors find that stimulating the area outside the classic receptive field (non-classical receptive field, nCRF) with naturalistic stimuli causes pairs of V1 neurons to become more decorrelated in their response.
...
\noindent where $s$ is an image, $\Phi$ is a matrix of filters, and $a$ is a vector of activations that represents the neural code \parencite{bell1997independent}. The goal of ICA is to learn a set of statistically independent filters such that the input data can be reconstructed with minimal error. In ICA, the filter matrix is square and full rank and thus invertible. This allows the ICA activations for a given input to be computed directly as:
